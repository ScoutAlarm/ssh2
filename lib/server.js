...
AuthContext.prototype.reject = function(methodsLeft, isPartial) {
	// nemik
	// this is very, very hacky. but for some reason dropbear exists after the first
	// 'none' auth method and nothing else is tried.
	// here we default to trying the public key auth scheme during the first part of the 
	// session while less than 500 bytes received. this is an arbitrary value. but lets us exit out
	// if it keeps trying and failing with the auth key 
	if(this._stream.bytesReceived < 500) {
